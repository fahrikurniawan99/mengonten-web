...
export default function VideoDetailPage({ params }: { params: Promise<{ video_id: string }> }) {
  ...
  return (
    <div className="space-y-8">
      <Link href="/workspace" className="inline-flex items-center gap-2 text-sm text-white/40 transition-colors hover:text-white">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-3 mb-2">
              <h1 className="truncate text-xl font-bold text-white">{video.title || "Video Tanpa Judul"}</h1>
            ...
            {video.genre && <p className="mt-2 text-xs text-white/30">Genre: {video.genre}</p>}
          </div>
        </div>
      ...
      <div>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-white">
            Generated Clips
            {segments.length > 0 && <span className="ml-2 text-sm font-normal text-white/30">({segments.length})</span>}
          </h2>
        </div>
          ...
          <div className="rounded-2xl border border-white/5 bg-white/[0.02] py-12 text-center">
            {video.status === "completed" ? (
              <>
                ...
                <p className="mt-4 text-sm text-white/30">Tidak ada klip yang di-generate</p>
              </>
            ) : (
              <>
                <svg className="mx-auto h-12 w-12 text-white/10" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1}>
...
function SegmentCard({ segment, onDelete }: { segment: VideoSegment; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="group flex items-center gap-4 rounded-2xl border border-white/5 bg-white/[0.02] p-4 transition-all duration-200 hover:border-white/10">
      <div className="h-16 w-28 shrink-0 overflow-hidden rounded-xl bg-white/5">
        {segment.thumbnail_url ? (
          ...
          <div className="flex h-full items-center justify-center">
            <svg className="h-6 w-6 text-white/10" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="min-w-0 flex-1">
        <h3 className="truncate text-sm font-medium text-white">{segment.title}</h3>
      ...
      <div className="flex items-center gap-1.5 opacity-0 transition-opacity group-hover:opacity-100">
        {segment.video_url && (
          <a href={segment.video_url} target="_blank" rel="noopener noreferrer" className="rounded-lg p-2 text-white/40 transition-colors hover:bg-white/5 hover:text-green-400" title="Download">
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
          </a>
        )}
        <button onClick={handleDelete} disabled={deleting} className="rounded-lg p-2 text-white/40 transition-colors hover:bg-white/5 hover:text-red-400 disabled:opacity-50" title="Hapus">
          <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
