...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  expired: { label: "Kadaluarsa", color: "bg-slate-100 text-slate-500 border-slate-200" },
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-right">
                    <p className="text-lg font-bold text-slate-900">{formatPrice(tx.amount)}</p>
                    {tx.plan?.duration_days && (
                      <p className="text-xs text-slate-400">{tx.plan.duration_days} hari</p>
                    )}
                  </div>
                </div>
              </div>
