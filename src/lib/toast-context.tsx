...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={{ showToast }}>
      ...
      {/* Toast container */}
      <div className="pointer-events-none fixed inset-x-0 top-0 z-[100] flex flex-col items-center gap-3 px-4 pt-6">
        {toasts.map((toast) => (
...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: () => void }) {
  ...
  return (
    <div
      className={`toast-enter pointer-events-auto flex w-full max-w-sm items-center gap-3 rounded-xl border px-4 py-3 shadow-lg ${borderColors[toast.type]} ${bgColors[toast.type]}`}
    >
      {icons[toast.type]}
